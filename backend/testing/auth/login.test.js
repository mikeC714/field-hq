...
describe("should flawlessly insert a new user and login said user with the mock credentials", () => {
	const mockUser = { first_name: "Jon", last_name: "Doe", email: "testemail2@gmail.com", password: "test_password" };
		...
		it("Test the entire route /api/auth/login, failing due to invalid email and credentials", async() => {
		...
		it("Test the entire route /api/auth/login", async() => {
			const response = await request(app)
				.post('/api/auth/login')
